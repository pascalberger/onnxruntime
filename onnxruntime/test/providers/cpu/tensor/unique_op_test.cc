...
void RunUniqueTest(const std::vector<int64_t>& X_dims,
  ...
  test.AddOutput<T>("Y", output_dims, T);
  test.AddOutput<T>("indices", indices_dims, indices);
  test.AddOutput<T>("inverse_indices", inverse_indices_dims, inverse_indices);
  test.AddOutput<T>("counts", counts_dims, counts);
